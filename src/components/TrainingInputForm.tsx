...
interface Props {
  ...
  onAddTraining: (data: { exercise: string; reps: number; sets: number }) => void;
}
...
function TrainingInputForm({ onAddTraining }: Props) {
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    // 簡単な入力チェック
    if (!exercise || isNaN(repsNum) || isNaN(setsNum) || repsNum <= 0 || setsNum <= 0) {
      alert('種目名、回数、セット数を正しく入力してください。');
      return; // 処理を中断
    }

    // App.tsx の handleAddTraining 関数を呼び出し、入力データを渡す
    onAddTraining({ exercise, reps: repsNum, sets: setsNum });
